					...
					<tbody>
						{phones.map((phone, index) => (
							<tr key={index} className="item-checout">
								<td className="first-column-checkout"><img
		...
		<div>
			...
			{R.not(isBasketEmpty) && <div>
				<button onClick={cleanBasket} className="btn btn-danger">
					<span className="glyphicon glyphicon-trash"></span>Clean cart
				</button>
				<button onClick={() => basketCheckout(phones)} className="btn btn-success">
					<span className="glyphicon glyphicon-envelope"></span>Checkout
				</button>
			</div>}
		</div>
	);
